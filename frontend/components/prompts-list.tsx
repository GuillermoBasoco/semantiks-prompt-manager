          ...
          {prompts.map(function (p) {
            return (
              <tr key={p.id} className="border-t">
                <td className="p-3 max-w-[240px] truncate" title={p.role}>
                  <a className="underline" href={`/prompts/${p.id}`}>{p.role}</a>
                </td>
                <td className="p-3 max-w-[260px] truncate" title={p.task}>{p.task}</td>
                <td className="p-3 space-x-1">
                  {p.tags.map(function (t, i) { return <span key={i} className="badge">{t}</span> })}
                </td>
                <td className="p-3"><span className={`badge ${p.status === 'active' ? '' : 'bg-gray-200 text-gray-600'}`}>{p.status === 'active' ? 'Active' : 'Inactive'}</span></td>
                ...
                <td className="p-3 text-right space-x-2">
                  ...
                  <button className="btn-primary px-3 py-1" onClick={function () { toggle(p.id, p.status) }}>{p.status === 'active' ? 'Deactivate' : 'Activate'}</button>
